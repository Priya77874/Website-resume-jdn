...
// --- SECURE CREDENTIALS (SALTED REVERSE BASE64) ---
// User: 10171029 -> Reversed + Salt -> Encoded
const DEFAULT_USER_ENC = "OTIwMTcxMDFfUkFKX1NFQ1VSRQ=="; 
// Email: rajkumar200406@gmail.com -> Reversed + Salt -> Encoded
const DEFAULT_MAIL_ENC = "bW9jLmxpYW1nQDYwNDAwMnJhbXVramFyX1JBSV9TRUNVUkU="; 
// Pass: Raj@200406 -> Reversed + Salt -> Encoded
const DEFAULT_PASS_ENC = "NjA0MDAyQGphUl9SQUpfU0VDVVJF"; 
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onAdminLogin, onGuestLogin }) => {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      if (activeTab === 'login') {
        ...
        // Use Secure Encoding Check
        const uEncoded = secureEncode(username);
        const mEncoded = secureEncode(username.toLowerCase());
        const pEncoded = secureEncode(password);

        if ((uEncoded === creds.user || mEncoded === DEFAULT_MAIL_ENC) && pEncoded === creds.pass) {
          onAdminLogin();
        } else {
  ...
  const recVerifyEmail = () => {
      const encodedEmail = secureEncode(recEmail.trim().toLowerCase());
  ...
  const recSaveNewPass = () => {
      ...
      const newPassEncoded = secureEncode(newPass);

      if (newPassEncoded === currentCreds.pass) {
          setRecError("New password cannot be same as old.");
          return;
      }

      const updatedCreds = {
          user: newUser ? secureEncode(newUser) : currentCreds.user,
          pass: newPassEncoded
      };
